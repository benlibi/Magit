...
public class MagitManager {
    ...
    public List<PR> getPrs(String userName, String repoName){
        prList.get(userName).computeIfAbsent(repoName, k -> new ArrayList<>());
        return prList.get(userName).get(repoName);
    }
...
//    }
    ...
    public void cancelPr(String currentUser,String repo,String id){
        List<PR> prs = prList.get(currentUser).get(repo);
        for(PR pr: prs){
    ...
    private String getChangedFiles(String src_branch, String trg_branch, String prAsker, String remoteUser, String srcBranch, String remoteBranch){
        String headCommit = getHeadCommitOfBranch(srcBranch);
        String headTrgCommit = Branch.getBranchCommitPointer(currentRepo.getRemote_path()+"/.magit/Branches/"+remoteBranch);
        return getChangesSring(headCommit, headTrgCommit);
    }

    public String getChangesSring(String firstCommitSha1, String secondCommitSha1) {
        Map<String, Blob> firstCommitFiles = getCommitFilesMap(firstCommitSha1, currentRepo.get_path());
        Map<String, Blob> secondCommitFiles = getCommitFilesMap(secondCommitSha1, currentRepo.getRemote_path());
    ...
    public void createPr(String src_branch, String trg_branch, String pr_msg, String prAsker) {
        ...
        String changedFilesString = getChangedFiles(src_branch,trg_branch, prAsker, remoteUser, src_branch, trg_branch);
        PR pr = new PR(prAsker, remoteUser, repoName, pr_msg, trg_branch, src_branch, changedFilesString);
        Map<String,List<PR>> user_repos = prList.get(remoteUser);
        if(!user_repos.containsKey(repoName)){
            prList.get(remoteUser).put(repoName,new ArrayList<>());
        }
        prList.get(remoteUser).get(repoName).add(pr);
    ...
    public synchronized void addCurrentUser(String userName) {
        ...
        if (rootDir.exists()) {
            ...
            if(!prList.containsKey(userName)){
                prList.put(userName,new HashMap<>());
            }
//            Utils.createUserFile(rootRepo,userFileName,userName);
    ...
    public boolean isRemote() {
        return currentRepo == null || currentRepo.isRemote;
    }
    ...
    protected ArrayList<String> getRemoteAvailableBranches() {
        ...
        File[] branchFiles = new File(this.currentRepo.getRemote_path().concat("/.magit/Branches")).listFiles();
    ...
    public void commit(String commitMsg, String commitSha1, boolean forceCommit, String committer) throws IOException {
        ...
        if (isChangesFound() || forceCommit) {
            ...
            if (currentCommit == null) {
                currentCommit = new Commit(commitMsg, rootFolder.getFolderSha1(), committer);
            } else {
                ...
                currentCommit = new Commit(commitMsg, rootFolder.getFolderSha1(), commitHistory, committer);
            }
    ...
    protected void createLocalRepository(String path, String repo_Name, String remote_user) throws IOException {
        Utils.createDir(path);
        Utils.createDir(path + "/.magit");
        //Utils.createDir(path + "/.magit/" + currentRepo.getRemote_name());
        Utils.createRepoFile(path + "/.magit", repo_Name, rootRepo + "/" + remote_user, remote_user);
        this.currentRepo = new Repository(path);
        this.currentRepo.createBlankRepository();
    }
    ...
    public void merge(String srcBranch, String trgBranch,String user) throws IOException {
    ...
    public Map<String, Blob> getCommitFilesMap(String commitSha1, String path) {
    ...
    private void copyFiles() throws IOException {
        ...
        Utils.copyFolderContent(this.currentRepo.getRemote_path() + "/.magit/Branches/",
                this.currentRepo.BRANCHES_DIR_PATH);
        //move head
...
//                this.currentRepo.BRANCHES_DIR_PATH + "/HEAD");
    ...
    private void logicPopulation() throws IOException {
        //create head branch
        String branchName = readBranchFile("HEAD");
        String headCommitSha1 = readBranchFile(branchName);
...
//        this.currentBranch = new Branch(branchName)
    ...
    public void clone(String repo_name, String remote_user, String currentUser) throws IOException {
        String path = rootRepo.concat("/" + currentUser + "/" + repo_name);
        createLocalRepository(path, repo_name, remote_user);
        copyFiles();
        logicPopulation();
    }
    ...
    private void copyBranchAndCommitsToRemoteRepo(String BranchName) throws IOException {
        String remoteHeadCommitSha1 = readBranchFile(BranchName);
        copyCommitsToRemote(remoteHeadCommitSha1);
        //copy branch file
        Utils.copyFile(this.currentRepo.BRANCHES_DIR_PATH + "/" + BranchName,
                this.currentRepo.getRemote_path().concat("/.magit/Branches/" + BranchName));
    }


    private void pullCommitsToLocalRepo(String remoteRepoBranchName) throws IOException {
        String remoteHeadCommitSha1 = readRemoteRepoBranchFile(remoteRepoBranchName);
        String localHeadCommitSha1 = readBranchFile(remoteRepoBranchName);
        if (!localHeadCommitSha1.equals(remoteHeadCommitSha1)) {
            copyCommits(remoteHeadCommitSha1);
            //copy branch file
            Utils.copyFile(this.currentRepo.getRemote_path().concat("/.magit/Branches/" + remoteRepoBranchName),
                    this.currentRepo.BRANCHES_DIR_PATH + "/" + remoteRepoBranchName);
            this.currentCommit = getCommitRep(remoteHeadCommitSha1);
    ...
    public void push() throws IOException {
        String branchName = this.currentBranch.getName();
        copyBranchAndCommitsToRemoteRepo(branchName);
